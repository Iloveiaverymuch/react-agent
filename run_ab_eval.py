...
Runs all 20 tasks under 3 configurations and produces:
    ab_results.jsonl       — raw results per task per config
    ab_comparison.md       — side-by-side table + write-up
...
TASKS = [
    ...
    {"id": "A4", "category": "single-tool",   "task": "Read the file notes.txt and tell me the value of the 'author' field.",                                              "pass_if": lambda r: "riadh" in r.lower()},
    {"id": "A5", "category": "single-tool",   "task": "Read notes.txt and tell me what 'stack' is listed.",                                                                "pass_if": lambda r: "python" in r.lower() and "anthropic" in r.lower()},
    # B: multi-hop
    {"id": "B1", "category": "multi-hop",     "task": "Read notes.txt, find the number in the 'lines' field, then compute that number squared.",                           "pass_if": lambda r: "144" in r},
    {"id": "B2", "category": "multi-hop",     "task": "Read notes.txt to find the week field, then compute the square root of that week number.",                          "pass_if": lambda r: "1.414" in r or "1.41" in r},   # fixed: sqrt(2) is correct
    ...
    {"id": "B4", "category": "multi-hop",     "task": "Read notes.txt, count how many key-value pairs it contains (lines with a colon), then multiply that count by 3.",  "pass_if": lambda r: "30" in r or "33" in r},   # accept 30 or 33 (counting ambiguity)
    ...
    {"id": "D5", "category": "adversarial",   "task": "Read notes.txt, then search for the goal mentioned in it, then calculate how many characters are in that goal string.", "pass_if": lambda r: any(c.isdigit() for c in r) and "max steps" not in r.lower()},
]
...
def write_comparison(results: list, output_dir: Path):
    ...
    for tid in task_ids:
        ...
        if n and f and t:
            changed = []
            if f["passed"] != n["passed"]: changed.append(f"fixed: {'✓' if f['passed'] else '✗'}")
            if t["passed"] != n["passed"]: changed.append(f"triggered: {'✓' if t['passed'] else '✗'}")
            if changed:
